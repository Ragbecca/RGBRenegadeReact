...
const LoginForm = () => {
  ...
  function handleSubmit(event) {
    ...
    login(loginRequest)
      .then((response) => {
        const { token, refreshToken, imgUrl, name, roles } = response.data;
        const data = parseJwt(token);
        const user = { data, token, refreshToken, roles, imgUrl, name };
    ...
    <div className="app-body-login">
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-item-btn">
          <button type="submit" className="btn btn-big btn-block btn-primary">
            Login
          </button>
        </div>
      </form>
